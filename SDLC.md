...
The repository's required status check is a single GitHub Actions job named exactly `check` (`.github/workflows/check.yml`), which runs the same two commands. Local gate and CI gate are intentionally identical here.
